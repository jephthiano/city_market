...
if(isset($_GET['val'])){
	...
	if(!empty($raw_val)){$id = content_data('user_notification_table','or_id',$raw_val,'or_id');}else{trigger_error_manual(404);}
}else{
	...
	<div class='j-row j-home-padding'style='padding-top:0px;'>
		...
		<div class='j-col l8 xl9 j-padding-flexible'>
			<div class='j-color4 j-account-height'>
					<?php
					if($id === false || content_data('user_notification_table','u_id',$id,'or_id') !== $u_id){  
						trigger_error_manual(404);
					}else{
						//change all notification status to seen
						$notification = new order('admin');
						$notification->or_id = $id;
						$notification->change_user_noti_status('change_read');
						...
						<div class='j-padding'>
							<?php
							$dat = multiple_content_data('user_notification_table','n_id',$u_id,'u_id',"AND or_id = {$id} ORDER BY n_id DESC",);
							if($dat !== false){
							foreach($dat AS $n_id){
								?>
								<div class='j-padding j-round j-color4'style='width:90%;max-width:400px;'>
									<div class='j-bolder'style='line-height: 30px;'>
										<span><?=ucwords(content_data('user_notification_table','n_title',$n_id,'n_id','','null'))?></span>
									</div>
									<div><?=ucfirst(content_data('user_notification_table','n_message',$n_id,'n_id','','null'))?></div>
									<div class='j-bolder j-text-color3 j-right j-small'>
										<?php $date = content_data('user_notification_table','n_regdatetime',$n_id,'n_id','','null');?>
										<?=show_date($date)?>
