...
<?php if(php_self('/index.php','home')){?>
   ...
   .done(function(s){
    ...
    .done(function(s){
     ...
     $("#top_category").html(s);
      <?php //get top store ?>
      $.ajax({type:'GET',url:dar+'get/gts/',cache:true})
      .done(function(s){
      $("#top_store").html(s);
         <?php // get top brand?>
         $.ajax({type:'GET',url:dar+'get/gtb/',cache:true})
         .done(function(s){
         $("#top_brand").html(s);
            <?php // get official store?>
            $.ajax({type:'GET',url:dar+'get/gos/',cache:true})
            .done(function(s){
            $("#official_store").html(s);
            })
         })
      })
     })
