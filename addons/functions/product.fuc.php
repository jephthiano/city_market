  ...
  if($numRow > 0){		// if a record is found
   while($stmt->fetch()){
      ...
      <div class='j-col s6 m4 xl2 j-padding-small'>
       <div class='j-color4'>
        ...
        <div class='j-color4'style='padding-left:5px;padding-right:5px;'>
          <div style='margin-top:5px;'><?=official_store($id,'product','small')?></div>
    ...
    <?php
     ...
     $token = content_data('order_table','or_token',$order_id,'or_id');
     $or_order_id = content_data('order_table','or_order_id',$order_id,'or_id');
     if($user === 'admin'){
      $url = file_location('admin_url','order/preview_orders/');
      $value = $token;
     }elseif($user === 'seller'){
      $url = file_location('seller_url','order/order_details/');
      $value = $or_order_id;
     }else{
    ...
    <?php
      ...
      $n_id = content_data('user_notification_table','n_id',$order_id,'or_id','ORDER BY n_id DESC');
      ?>
     ...
     <div class='j-padding'>
      <div class='j-row j-border-color7'style='border-bottom:solid 1px;'>
       ...
       <div class='j-col s9 j-padding'>
        <div>
         <span class='j-bolder'><?=ucwords(content_data('user_notification_table','n_title',$n_id,'n_id','','null'))?></span>
         <span class='j-bolder j-right j-text-color5'><?=show_date(content_data('user_notification_table','n_regdatetime',$n_id,'n_id'))?></span>
        </div>
        <div>
         <span><?=text_length(ucfirst(content_data('user_notification_table','n_message',$n_id,'n_id','','null')),70,'dots')?></span>
         <?php
         $unread = get_numrow('user_notification_table','u_id',$u_id,"return",'round',"AND or_id = {$order_id} AND n_status != 'read'");
    ...
    <?php
     ...
     $placed_time = content_data('order_table','or_regdatetime',$order_id,'or_id');
     if($user === 'admin'){
      $url = file_location('admin_url','product/preview_product/'.addnum($id));
     }elseif($user === 'seller'){
      $url = '#';
     }else{
      $url = file_location('home_url','product/'.addnum($id));
     }
     ?>
      ...
      <div class='j-row'>
       ...
       <div class='j-col m3'>
        ...
        if($user === 'admin'){
         ...
         ?>
         <a href="<?=file_location('seller_url','order/track/'.content_data('order_table','or_order_id',$order_id,'or_id'))?>"style='margin-bottom:5px;'>
           <div class='j-color1 j-padding j-center j-round j-small j-bolder'>SEE ORDER HISTORY</div>
          </a>
         <?php
        }else{
     ...
     <div class='j-color4 j-padding-small'>
      <div class='j-row'>
       <div class='j-col l5 j-padding-small'>
        ...
        <div style='margin-top:12px;'><?=official_store($id,'product')?></div>
       </div>
...
function is_avail($id){
  ...
  $seller_id = content_data('product_table','s_id',$id,'p_id');
  $seller_status = content_data('seller_table','s_status',$seller_id,'s_id');
  if($status === 'available' && (get_total_available($id) > 0) && $seller_status === 'active'){
    return true;
  }else{
    return false;
  }
