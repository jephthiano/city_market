...
function get_media($type,$id,$media='no_media'){
 ...
 @$conn = dbconnect('admin','PDO');
  ...
  $sql = "SELECT sm_link_name,sm_extension FROM seller_media_table WHERE s_id = :id LIMIT 1";
  $stmt = $conn->prepare($sql);
  $stmt->bindParam(':id',$id,PDO::PARAM_INT);
  $stmt->bindColumn('sm_link_name',$link_name);
  $stmt->bindColumn('sm_extension',$extension);
 }elseif($type === 'product'){
  ...
  $sql = "SELECT scm_link_name,scm_extension FROM sub_category_media_table WHERE sc_id = :id";
  $stmt = $conn->prepare($sql);
  $stmt->bindParam(':id',$id,PDO::PARAM_INT);
  $stmt->bindColumn('scm_link_name',$link_name);
  $stmt->bindColumn('scm_extension',$extension);
 }else{
  return 'home/no_media.png';
   ...
   if(file_exists(file_location('media_path',$file)) && is_file(file_location('media_path',$file))){
    return $file;
     ...
     if($type === 'admin' || $media === 'human'){return 'home/avatar.png';}else{return 'home/no_media.png';}
   }
		}
	}else{
  if($type === 'admin' || $media === 'human'){return 'home/avatar.png';}else{return 'home/no_media.png';}
