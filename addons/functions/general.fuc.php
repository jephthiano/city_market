...
//no user surf starts
if(!strstr($_SERVER['SERVER_NAME'],'admin.') && !strstr($_SERVER['SERVER_NAME'],'seller.')){no_user_surf();}
...
//no seller surf starts
if(strstr($_SERVER['SERVER_NAME'],'seller.')){no_seller_surf();}
function no_seller_surf(){
 if(get_json_data('surfing','seller_act') == 0 || get_json_data('all','seller_act') == 0){
  die(require_once(file_location('home_path','error/no_surfing.enc.php')));
 }
}
//no seller surf ends
...
function get_header($header,$button='settings',$size=30,$type='home_url'){
 if(empty($size)){$size=30;}if(empty($type)){$type='home_url';}
 if($button === 'settings'){
  ...
  $btn = "<a href='".file_location($type,$button)."'><span style='margin-right:30px;'class='j-btn''><i class='j-xlarge ".icon('arrow-left')."'></i></span></a>";
 }
 ?>
