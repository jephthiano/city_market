	...
	if(isset($navigation) && $navigation === 'payment'){
		...
		<div id="nav"class="j-bar j-color4 j-text-color1 j-fixed-top j-card j-home-padding"style="margin:0px;font-size:12px;z-index:1;height:50px;width:100%;overflow-y:hidden">
			...
			<div class="j-search-div j-bar-item j-hide-small"style='padding:7px 0px 5px 12px;position:relative;display:inline-block;'>
				<input type="search"name="txtsearch"id="txtsearch"class="searchinput j-input j-small j-border-2 j-border-color5 j-round j-search-width"
					value='<?=php_self('/search.enc.php','home')?$searchtext:"";?>'placeholder="Search <?=ucwords(get_xml_data('company_name'))?>"
		...
		<div class="j-hide-large j-hide-xlarge j-hide-medium j-card-4 j-color4 j-fixed-nav" style="margin:0px; font-size:12px;z-index:1">
			<div class="j-row-padding j-center" style="padding: 10px 0px">
				<div class="j-col s2">
					<a id='home2'href="<?= file_location('home_url','');?>" <?php if(php_self('/index.php','home')){?>onclick="iauulr('home')";<?php }?>>
						<span class="j-small <?=php_self('/index.php','home')?'j-text-color1':'j-text-color7';?>"><i class="j-large <?=icon('home');?>"style='display:block'></i>Home</span>
					</a>
				</div>
				<div class="j-col s2">
					<a id=''href="<?= file_location('home_url','inbox/');?>"class='j-display-container'style='display:relative;'>
						<span class="j-small <?=php_self('/inbox/index.php','home')?'j-text-color1':'j-text-color7';?>">
							<i class="j-large <?=icon('envelope');?>"style='display:block'></i>Inbox
							<?php
							if(isset($_SESSION['user_id'])){
								?>
								<span class='ntf j-circle <?=php_self('/inbox/index.php','home')?'j-color3':'j-color1';?> j-small'style='width:20px;height:20px;position:absolute;top:-5px;right:-9px;'>0</span>
								<?php
							}
							?>
						</span>
				...
				<div class="j-col s3">
					<a id=''href="<?= file_location('home_url','category/');?>">
						<span class="j-small <?=php_self('/category/index.php','home')?'j-text-color1':'j-text-color7';?>"><i class="j-large <?=icon('list-ul');?>"style='display:block'></i>Category</span>
					</a>
				</div>
				<div class="j-col s2"style='display:relative;'>
					<a id=''href="<?= file_location('home_url','cart/');?>"class='j-display-container'>
						<span class="j-small <?=php_self('/cart.enc.php','home')?'j-text-color1':'j-text-color7';?>">
							<i class="j-large <?=icon('shopping-cart');?>"style='display:block'></i>Cart
							<span class='crt j-circle <?=php_self('/cart.enc.php','home')?'j-color3':'j-color1';?> j-small'style='width:20px;height:20px;position:absolute;top:-5px;right:-9px;'>0</span>
						</span>
					</a>
				</div>
