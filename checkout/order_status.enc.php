...
$add = "AND or_status = 'cart' AND order_table.p_id = product_table.p_id AND p_status = 'available' AND product_table.s_id = seller_table.s_id AND seller_table.s_status = 'active' ORDER BY or_id DESC";
//if token is not empty or if token is not in order table or if token is not in orderer table
if(($token === 'no_token') || (content_data('order_table,product_table,seller_table','or_token',$token,'or_token',$add) === false)|| (content_data('orderer_table','or_token',$token,'or_token') === false)){
