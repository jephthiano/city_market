...
if(isset($_POST)){
	...
	$cat = ($_POST['ct']);
	if($cat == 0){$category = 0;}elseif(empty($cat)){$missing['cte'] = "* category cannot be empty";}else{$category = strtolower(test_input($cat));}
	
	// validating and sanitizing sub category
	$sbcat = ($_POST['sct']);
	if($sbcat == 0){$sub_category = 0;}elseif(empty($sbcat)){$missing['scte'] = "* sub category cannot be empty";}else{$sub_category = strtolower(test_input($sbcat));}
		...
		if(empty($missing) && empty($error)){
			if($file2 === "larger"){ // if file is larger tha expected echo error
				...
				if($insert == true && is_numeric($insert)){
					$data["status"] = 'success';$data["message"] = file_location('seller_url','product/preview_product/'.addnum($insert));
					//INSERT SELLER LOG
					$log = new log('admin');
					...
					$data["status"] = 'fail';$data["message"] = 'Sorry!!!<br>You have a product with the same name existing in product catalog, please try and verify before adding product';
				}
			}else{
