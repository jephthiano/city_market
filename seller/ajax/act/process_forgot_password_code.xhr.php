...
if(isset($_POST) && isset($_POST['cde'])){
	...
	}elseif(time_validity(300,content_data('seller_emailcode_table','c_regdatetime',$cookie_email,'c_email'))){
	...
	if(empty($error) and empty($missing)){
		...
		if(insert_delete_seller_code('update',$cookie_email,$code)){
			$data["status"] = 'success';$data["message"] = file_location('seller_url','forgot_password/enter_password/');
		}else{
