...
if(isset($_POST)  && isset($_POST['ema'])){
	...
	}elseif(content_data('seller_table','s_email',$ema,'s_email') === false){
	...
	if(empty($error) and empty($missing)){
		...
		insert_delete_seller_code('delete',$email);
		
		//SET CODE INTO DATABASE AND SET TOKEN
		$code = generate_code();
		set_seller_forgot_password_token($email,$code);
		if(insert_delete_seller_code('insert',$email,$code) === true){
			...
			$mail->p_message = email_code_message($code,'seller');
			...
			if($mailsent){
				$data["status"] = 'success';$data["message"] = file_location('seller_url','forgot_password/enter_code/');
			}else{
				//DELETE ANY CODE IN DATABASE AND TOKEN
				insert_delete_seller_code('delete',$email);
