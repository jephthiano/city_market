...
$location = file_location('seller_url','forgot_password/');
$location2 = file_location('seller_url','forgot_password/enter_code/');
$page_url = file_location('seller_url','forgot_password/');
//for meta
	...
	<center>
		...
		<div class="j-round j-panel j-border j-border-color5" style="width:100%; max-width:400px; height:auto;">
			...
			<span class="j-text-color1 j-large "style='line-height:50px'>
				<a href='<?=file_location('seller_url','')?>'>
			...
			if($type === 'enter_email'){
				...
				<form id='eemfrm'>
					...
					<br><br>
					<a class="j-text-color3 j-center j-bolder"href="<?= file_location('seller_url','login/');?>">Back to Login In</a><br><br>
				</form>
				<?php
				...
				if(empty($cookie_email) || ($cookie_email === false) || empty($cookie_code) || ($cookie_code === false) || ($cookie_code !== $db_code)){
					insert_delete_seller_code('delete',$cookie_email);die(header("Location:$location"));
				}
				?>
				<form id='ecdfrm'>
