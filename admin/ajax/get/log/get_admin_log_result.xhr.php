...
if(isset($_POST['s']) && isset($_POST['pg'])){
	...
	if(!empty($searchtext)){ // if the search text is not empty
		...
		$sql = "SELECT l_id,l_brief,l_details,l_regdatetime,ad_id,ad_username FROM admin_log_table
		...
		$sql = "SELECT l_id,l_brief,l_details,l_regdatetime,ad_id,ad_username FROM admin_log_table ORDER BY l_id DESC LIMIT $start,$display";
		$stmt = $conn->prepare($sql);
				...
				<table class="j-table-all j-border-0">
					...
					while($stmt->fetch()){
						...
						<tr class="j-border-0">
							...
							<td><a href='<?= file_location('admin_url','log/preview_admin_log/'.addnum($id).'/')?>'><i class="j-large <?= icon('eye');?>"></i></a></td>
						</tr>
						<?php
					}// end of while
					?>
