...
$page_url = file_location('admin_url','log/preview_admin_log/'.$_GET['page']);
$page = "PREVIEW ADMIN LOG";
...
if(isset($_GET['page']) AND is_numeric($_GET['page'])){ //getting the value of the get 
	...
	if(!empty($cid)){	
		$id = content_data('admin_log_table','l_id',$cid,'l_id');
	}else{
	...
	<div class="j-col s12 l10"id='mainbody'>
		...
		<div id="maincol"class='j-main-col'>
			<div class=''style='margin-bottom:20px;'>
				<h2 class='j-text-color1 j-padding j-color4'><b>ADMIN LOG DATA</b></h2>
			</div>
			<center>
				<div class='j-padding'>
					<a href="<?=file_location('admin_url','log/admin_log/')?>" class="j-btn j-color1 j-left j-round j-card-4 j-bolder">Show All Admin Logs</a>
				</div>
			</center>
				...
				<div class="j-container">
					...
					if($id === false){
						...
						$admin = content_data('admin_table','ad_username',content_data('admin_log_table','ad_id',$id,'l_id','','null'),'ad_id','','null');
						?>
						...
						<div class='j-color4 j-padding'>
							<div class="j-row-padding j-padding"><div class='j-col s4 j-text-color5'><b>ACTION DOER:</b></div>
								<div class='j-col s8'><?=ucfirst(content_data('admin_log_table','ad_username',$id,'l_id','','null'))?></div>
							</div>
							<div class="j-row-padding j-padding">
								<div class='j-col s4 j-text-color5'><b>BRIEF</b></div>
								<div class='j-col s8'><?=ucfirst(content_data('admin_log_table','l_brief',$id,'l_id','','null'))?></div>
							</div>
							<div class="j-row-padding j-padding">
								<div class='j-col s4 j-text-color5'><b>DETAILS:</b></div>
								<div class='j-col s8'><?=ucfirst($admin.' '. content_data('admin_log_table','l_details',$id,'l_id','','null'))?></div>
							</div>
							<div class="j-row-padding j-padding">
								<div class='j-col s4 j-text-color5'><b>IP ADDRESS:</b></div>
								<div class='j-col s8'><?=content_data('admin_log_table','l_ip_address',$id,'l_id','','null')?></div>
							</div>
							<div class="j-row-padding j-padding">
								<div class='j-col s4 j-text-color5'><b>DATE:</b></div>
								<div class='j-col s8'><?=showdate(content_data('admin_log_table','l_regdatetime',$id,'l_id','','null'),'')?></div>
							</div>
							<div class="j-row-padding j-padding">
