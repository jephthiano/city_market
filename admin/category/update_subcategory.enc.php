...
$page_url = file_location('admin_url','category/update_subcategory/'.@$_GET['page']);
$page = "UPDATE SUB CATEGORY";
...
<div class="j-row">
	...
	<div class="j-col s12 l10"id='mainbody'>
		...
		<div class='j-padding'>
			<h2 class='j-text-color1 j-padding j-color4'><b>UPDATE SUB CATEGORY</b></h2>
		</div>
		<div class='j-padding'>
			...
			if($id === false){
				...
				<div class='j-padding'>
					<div id='preview'class='j-border-color5 j-border j-circle j-color5 j-vertical-center-container j-clickable'style='width:100px;height:100px;'
						 onclick="$('#subcategory<?=$id?>_pics_modal').fadeIn('slow');">
						<img src='<?=file_location('media_url',get_media('subcategory',$id))?>'alt=''class='j-round j-card-4'style='width:100px;height:100px;border:solid 2px white'>
						<span class='j-text-color4 j-bold j-vertical-center-element'style='font-size:40px;'>+</span>
					</div>
					<?php image_modal('subcategory',$id)?>
				</div>
				<form onsubmit="event.preventDefault();"class=''id='upsscg'>
					<div class='j-row'>
						<div class='j-col m6 j-padding'>
							<label class="j-large j-text-color7"><b>Sub category:</b> <span class='j-text-color1 mg'id="cte">*</span></label>
							<input type="text"class="j-input j-medium j-border-2 j-border-color5 j-round"placeholder="Sub category"maxlength='50'
								name="ct"id="ct"value="<?=(content_data('sub_category_table','sc_sub_category',$id,'sc_id'))?>"style="width:100%;"/>
							...
							<input type="text"class="j-input j-medium j-border-2 j-border-color5 j-round"placeholder="Icon"maxlength='50'
								name="ic"id="ic"value="<?=content_data('sub_category_table','sc_icon',$id,'sc_id')?>"style="width:100%;"/><br>
						</div>
					</div>
					...
					<div class='j-margin'>
						<button type='submit'id='sbtn'class="j-btn j-medium j-color1 j-round j-bolder">Update Sub Category</button>
					</div>
				</form>
				<?php
			}
