...
if(isset($_POST) && isset($_POST['cde'])){
	$cookie_email = get_user_forgot_password_token('email');
	$cookie_code = get_user_forgot_password_token('code');
	$db_code = content_data('user_emailcode_table','c_code',$cookie_email,'c_email');
	...
	if(empty($cookie_email) || $cookie_email === false || empty($cookie_code) || $cookie_code === false || $cookie_code !== $db_code){
		$error[] = 'error';
		insert_delete_user_code('delete',$cookie_email);
	...
	}elseif(time_validity(300,content_data('user_emailcode_table','c_regdatetime',$cookie_email,'c_email'))){ //if time has not exceed 5 mins
	...
	if(empty($error) and empty($missing)){
		...
		if(insert_delete_user_code('update',$cookie_email,$code)){
